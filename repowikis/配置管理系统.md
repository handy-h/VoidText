...
## 简介
本文件为 txtCleaning 的配置管理系统提供全面、可操作的配置文档。内容涵盖环境变量配置、处理规则配置、模型参数配置，解释配置加载机制与验证流程，明确各参数的含义、默认值与取值范围；同时给出配置文件格式与示例、热更新与运行时修改能力说明、最佳实践与安全注意事项、配置继承与覆盖规则、常见场景与故障排除指南，以及配置验证工具与测试方法。
...
- Web 启动入口：cmd/txtclean/main.go
- 文档说明：doc/06-配置说明.md

```mermaid
graph TB
subgraph "应用入口"
M["cmd/txtclean/main.go<br/>启动HTTP服务"]
end
subgraph "配置层"
...
章节来源
- [config.go:14-46](file://internal/config/config.go#L14-L46)
- [config.go:48-122](file://internal/config/config.go#L48-L122)
...
章节来源
- [config.go:71-112](file://internal/config/config.go#L71-L112)
- [06-配置说明.md:30-61](file://doc/06-配置说明.md#L30-L61)
- [06-配置说明.md:114-123](file://doc/06-配置说明.md#L114-L123)
...
章节来源
- [config.go:190-203](file://internal/config/config.go#L190-L203)
- [config_test.go:88-122](file://internal/config/config_test.go#L88-L122)
- [rules.go:35-86](file://internal/processor/rules/rules.go#L35-L86)

## 结论
txtCleaning 的配置系统通过清晰的环境变量与文件配置分层，提供了灵活且可验证的配置能力。全局配置保证系统启动一致性，文件级规则配置支持细粒度控制。配合完善的测试与文档，用户可以在不同部署环境中快速、安全地配置与运行系统。
...
章节来源
- [06-配置说明.md:7-81](file://doc/06-配置说明.md#L7-L81)
- [config.go:71-112](file://internal/config/config.go#L71-L112)
...
章节来源
- [06-配置说明.md:82-113](file://doc/06-配置说明.md#L82-L113)
- [rules.json:1-18](file://internal/processor/rules.json#L1-L18)
- [pipeline.go:119-119](file://internal/processor/pipeline.go#L119-L119)
...
章节来源
- [pipeline.go:48-71](file://internal/processor/pipeline.go#L48-L71)
- [config.go:98-112](file://internal/config/config.go#L98-L112)
- [06-配置说明.md:114-123](file://doc/06-配置说明.md#L114-L123)