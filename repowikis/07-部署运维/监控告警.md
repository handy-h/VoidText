...
## 简介
本文件面向VoidText项目的运维与开发团队，提供一套完整的监控告警体系文档。内容覆盖日志采集与分析、性能指标监控与阈值设定、系统健康检查与可用性监控、关键指标（错误率、响应时间、吞吐量等）监控、告警通知渠道配置（邮件、微信、钉钉等）、监控仪表板搭建与可视化、故障自动恢复与降级策略，以及监控数据的存储与归档方案。目标是在保障系统稳定运行的同时，降低人工巡检成本，提升问题发现与处置效率。

## 项目结构
VoidText采用前后端分离架构，后端由Go + Gin提供REST API，内部模块包括配置、数据库、文件处理、处理器流水线、外部API调用、日志等。前端为单页应用（SPA），通过HTTP REST API与后端交互。
...
- [02-系统架构.md:3-31](../doc/02-系统架构.md#L3-L31)
...
章节来源
- [config.go:49-122](../internal/config/config.go#L49-L122)
- [logger.go:68-141](../internal/logging/logger.go#L68-L141)
- [db.go:15-69](../internal/database/db.go#L15-L69)
- [chunk_cache_repo.go:422-478](../internal/database/chunk_cache_repo.go#L422-L478)
- [evolver_monitor.go:43-177](../internal/processor/evolver_monitor.go#L43-L177)
- [server.go:10-57](../web/backend/server.go#L10-L57)
...
- [evolver.py:143-168](../scripts/evolver.py#L143-L168)
...
章节来源
- [logger.go:12-37](../internal/logging/logger.go#L12-L37)
- [logger.go:68-141](../internal/logging/logger.go#L68-L141)
...
- [logger.go:165-173](../internal/logging/logger.go#L165-L173)
...
- [06-配置说明.md:69-81](../doc/06-配置说明.md#L69-L81)
...
章节来源
- [chunk_cache_repo.go:452-478](../internal/database/chunk_cache_repo.go#L452-L478)
- [logger.go:165-173](../internal/logging/logger.go#L165-L173)
...
- [02-系统架构.md:199-205](../doc/02-系统架构.md#L199-L205)
- [README.md:122-126](../README.md#L122-L126)
...
```mermaid
graph TB
App["VoidText应用"] --> Gin["github.com/gin-gonic/gin"]
...
章节来源
- [main.go:38-43](../cmd/voidtext/main.go#L38-L43)
- [db.go:15-69](../internal/database/db.go#L15-L69)
- [06-配置说明.md:69-81](../doc/06-配置说明.md#L69-L81)
- [README.md:122-126](../README.md#L122-L126)

## 结论
通过统一的日志结构化输出、基于数据库统计的关键指标监控、Evolver自进化与重试队列降级策略，以及完善的健康检查与告警通知，VoidText项目可在生产环境中实现可观测性与高可用性。建议尽快落地日志采集与指标采集，完善仪表板与告警规则，并制定数据归档与清理策略，以支撑长期稳定运行。
...
- [server.go:28-54](../web/backend/server.go#L28-L54)
- [06-配置说明.md:62-81](../doc/06-配置说明.md#L62-L81)