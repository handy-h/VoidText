# 快速开始

<cite>
**本文引用的文件**
- [go.mod](../go.mod)
- [cmd/voidtext/main.go](../cmd/voidtext/main.go)
- [internal/config/config.go](../internal/config/config.go)
- [internal/database/db.go](../internal/database/db.go)
- [web/backend/server.go](../web/backend/server.go)
- [web/backend/handlers/files.go](../web/backend/handlers/files.go)
- [web/frontend/index.html](../web/frontend/index.html)
- [internal/processor/pipeline.go](../internal/processor/pipeline.go)
- [internal/processor/prompt_manager.go](../internal/processor/prompt_manager.go)
- [internal/processor/model_repairer.go](../internal/processor/model_repairer.go)
- [internal/processor/evolver_monitor.go](../internal/processor/evolver_monitor.go)
- [scripts/evolver.py](../scripts/evolver.py)
- [Makefile](../Makefile)
- [scripts/run.sh](../scripts/run.sh)
- [testing/smoking_testing/run_smoke_tests.sh](../testing/smoking_testing/run_smoke_tests.sh)
- [.gitignore](../.gitignore)
- [doc/06-配置说明.md](../doc/06-配置说明.md)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本指南面向首次接触 VoidText 的用户，帮助你在最短时间内完成环境准备、项目编译与运行，并掌握上传文件、触发处理流程、查看结果等核心操作。同时提供常见问题排查与 Docker 部署对比，便于你选择适合的部署方式。

## 项目结构
项目采用模块化组织，后端基于 Gin Web 框架，前端为静态页面，核心处理逻辑位于内部模块，数据库使用 SQLite，配置通过 .env 文件加载。

```mermaid
graph TB
subgraph "入口与运行"
M["cmd/voidtext/main.go<br/>应用入口"]
end
subgraph "配置层"
C["internal/config/config.go<br/>.env 加载与默认值"]
end
subgraph "数据库层"
D["internal/database/db.go<br/>SQLite 初始化与表结构"]
end
subgraph "Web 层"
S["web/backend/server.go<br/>路由注册与CORS"]
H["web/backend/handlers/files.go<br/>文件相关API"]
end
subgraph "前端"
F["web/frontend/index.html<br/>静态页面与交互"]
end
subgraph "处理流水线"
P["internal/processor/pipeline.go<br/>清洗/检测/修复/审核/生成"]
end
M --> C
M --> D
M --> S
S --> H
H --> D
H --> P
S --> F
```

**图表来源**
- [cmd/voidtext/main.go:18-61](../cmd/voidtext/main.go#L18-L61)
- [internal/config/config.go:49-122](../internal/config/config.go#L49-L122)
- [internal/database/db.go:16-69](../internal/database/db.go#L16-L69)
- [web/backend/server.go:11-57](../web/backend/server.go#L11-L57)
- [web/backend/handlers/files.go:18-71](../web/backend/handlers/files.go#L18-L71)
- [web/frontend/index.html:1-230](../web/frontend/index.html#L1-L230)
- [internal/processor/pipeline.go:105-158](../internal/processor/pipeline.go#L105-L158)

**章节来源**
- [go.mod:1-54](../go.mod#L1-L54)
- [cmd/voidtext/main.go:18-61](../cmd/voidtext/main.go#L18-L61)
- [internal/config/config.go:49-122](../internal/config/config.go#L49-L122)
- [internal/database/db.go:16-69](../internal/database/db.go#L16-L69)
- [web/backend/server.go:11-57](../web/backend/server.go#L11-L57)
- [web/backend/handlers/files.go:18-71](../web/backend/handlers/files.go#L18-L71)
- [web/frontend/index.html:1-230](../web/frontend/index.html#L1-L230)
- [internal/processor/pipeline.go:105-158](../internal/processor/pipeline.go#L105-L158)

## 核心组件
- 应用入口与生命周期：负责加载配置、初始化数据库、启动 HTTP 服务器、优雅关闭。
- 配置系统：支持 .env 与环境变量覆盖，提供默认值与校验。
- 数据库：SQLite，自动建表、索引与 WAL 优化。
- Web 服务：Gin 路由，CORS 放通，静态资源托管。
- 文件处理：上传、列表、详情、内容、下载、删除、恢复、规则更新、运行处理、状态与审核、报告。
- 处理流水线：清洗 → 向量检测 → LLM 修复 → 审核 → 生成最终文件。
- **提示词管理**：动态提示词加载、多版本管理、热重载、自进化优化。

**章节来源**
- [cmd/voidtext/main.go:18-61](../cmd/voidtext/main.go#L18-L61)
- [internal/config/config.go:49-122](../internal/config/config.go#L49-L122)
- [internal/database/db.go:16-69](../internal/database/db.go#L16-L69)
- [web/backend/server.go:11-57](../web/backend/server.go#L11-L57)
- [web/backend/handlers/files.go:18-71](../web/backend/handlers/files.go#L18-L71)
- [internal/processor/pipeline.go:105-158](../internal/processor/pipeline.go#L105-L158)
- [internal/processor/prompt_manager.go:17-76](../internal/processor/prompt_manager.go#L17-L76)

## 架构概览
下面的时序图展示了从浏览器访问到后端处理的关键交互。

```mermaid
sequenceDiagram
participant U as "用户浏览器"
participant FE as "前端页面<br/>index.html"
participant API as "Web服务<br/>server.go"
participant H as "文件处理器<br/>handlers/files.go"
participant DB as "数据库<br/>db.go"
participant PL as "处理流水线<br/>pipeline.go"
U->>FE : 打开首页
FE->>API : GET /
API-->>FE : 返回静态页面
U->>API : POST /api/files/upload (上传.txt)
API->>H : 调用 UploadFile
H->>DB : 创建文件记录/版本记录
H-->>U : 返回 {md5, message}
U->>API : POST /api/files/ : md5/run (触发处理)
API->>H : 调用 RunAllSteps
H->>PL : 按步骤执行清洗/检测/修复/审核/生成
PL->>DB : 写入处理日志/版本/审核项
H-->>U : 返回处理进度/状态
U->>API : GET /api/files/ : md5/status
API->>H : 调用 GetFileStatus
H->>DB : 查询文件状态
H-->>U : 返回 {status, progress, currentStep}
U->>API : GET /api/files/ : md5/report
API->>H : 调用 GetProcessingReport
H->>DB : 生成报告数据
H-->>U : 返回报告
```

**图表来源**
- [web/frontend/index.html:1-230](../web/frontend/index.html#L1-L230)
- [web/backend/server.go:28-54](../web/backend/server.go#L28-L54)
- [web/backend/handlers/files.go:18-71](../web/backend/handlers/files.go#L18-L71)
- [internal/database/db.go:89-222](../internal/database/db.go#L89-L222)
- [internal/processor/pipeline.go:105-158](../internal/processor/pipeline.go#L105-L158)

## 详细组件分析

### 环境与依赖准备
- Go 版本要求：项目使用 Go 1.25.0。
- 依赖管理：使用 go.mod 管理依赖；首次运行可先执行 go mod tidy/verify。
- 项目管理：提供 `Makefile`，涵盖编译、运行（开发/生产）、清理等常用操作。

建议步骤
- 安装 Go 1.25 或以上版本。
- 在项目根目录执行 `go mod tidy` 与 `go mod verify`。
- 查看可用命令：`make help`。
- 编译项目：`make build`。
- 开发运行：`make dev`（控制台打印日志，便于调试）。
- 生产运行：`make run`（需先执行 `make build`）。

**章节来源**
- [Makefile](../Makefile)
- [go.mod:3](../go.mod#L3)
- [scripts/run.sh:5-13](../scripts/run.sh#L5-L13)

### 配置与 .env
- 配置加载顺序：优先从可执行文件所在目录加载 .env，失败再回退到工作目录。
- 关键配置项：端口、数据目录、最大文件大小、备份保留天数、清洗/检测/修复开关与参数、Evolver 开关等。
- **提示词配置**：支持文件系统与数据库两种提示词来源，可配置热重载与自进化监控。
- 默认值与兼容：提供默认值与旧配置映射，确保平滑升级。
- 目录结构：DATA_DIR 下自动创建 uploads/backups/temp 等子目录。

建议步骤
- 复制模板文件：cp .env.template .env。
- 按需修改 .env 中的 PORT、DATA_DIR、模型相关参数等。
- **提示词配置**：
  - 启用自进化：设置 `ENABLE_EVOLVER=true`
  - 调整阈值：`EVOLVER_ERROR_RATE_THRESHOLD=0.2`（错误率 >20% 触发优化）
  - 调整阈值：`EVOLVER_HIT_RATE_THRESHOLD=0.3`（命中率 <30% 触发优化）
- 启动后可在日志中看到实际加载的环境变量值。

**章节来源**
- [internal/config/config.go:49-122](../internal/config/config.go#L49-L122)
- [doc/06-配置说明.md:1-146](../doc/06-配置说明.md#L1-L146)
- [.gitignore:37-46](../.gitignore#L37-L46)

### 数据库初始化
- 数据库类型：SQLite（modernc.org/sqlite）。
- 初始化要点：创建数据目录、打开数据库、启用 WAL 模式、设置同步级别与缓存大小、设置连接池与生命周期、创建表与索引。
- 表结构：files、versions、review_items、processing_logs、chunk_repair_cache、retry_queue、prompt_versions 等。
- **提示词版本表**：`prompt_versions` 表存储多版本提示词，支持自进化优化与版本管理。

建议步骤
- 启动应用后自动创建数据库文件与表结构。
- 如需迁移或清理，可直接删除 DATA_DIR 下的 cleaning.db 并重启应用。
- **查看提示词版本**：`SELECT * FROM prompt_versions WHERE prompt_name='novel_repair' ORDER BY created_at DESC;`

**章节来源**
- [internal/database/db.go:16-69](../internal/database/db.go#L16-L69)
- [internal/database/db.go:89-222](../internal/database/db.go#L89-L222)
- [internal/database/db.go:185-198](../internal/database/db.go#L185-L198)

### Web 服务与路由
- 框架：Gin。
- CORS：允许本地开发域名与方法头。
- 静态资源：/static 指向前端静态目录。
- 根路径：返回前端 index.html。
- API 分组：/api 下提供文件、规则、处理流程相关接口。

建议步骤
- 默认监听端口可在 .env 中配置（默认 8080）。
- 启动后访问 http://localhost:端口 查看前端页面。

**章节来源**
- [web/backend/server.go:11-57](../web/backend/server.go#L11-L57)

### 文件处理 API
- 上传：支持 .txt 文件，自动计算 MD5，去重与中间版本识别。
- 列表/详情/内容/下载/删除：对文件记录与内容进行 CRUD。
- 规则：支持按文件维度更新清洗/检测/修复规则。
- 处理：触发清洗、检测、修复、审核、生成等步骤，支持恢复与批量审核。
- 报告：获取处理报告。

建议步骤
- 上传文件后获得 md5。
- 调用运行接口触发处理。
- 通过状态接口轮询进度。
- 审核完成后生成最终文件并下载。

**章节来源**
- [web/backend/handlers/files.go:18-71](../web/backend/handlers/files.go#L18-L71)
- [web/backend/handlers/files.go:253-393](../web/backend/handlers/files.go#L253-L393)

### 处理流水线
- 步骤：cleaning → indexing → llm_fix → review → finalizing。
- 进度计算：每个步骤贡献 20%，结合步骤内进度。
- 中间产物：每步生成中间版本文件并记录版本信息。
- 审核：将检测/修复产生的变更转化为审核项，支持逐条审批/拒绝/编辑/恢复与批量操作。
- 最终生成：合并已批准/编辑的变更，生成最终文件并记录版本。

建议步骤
- 在前端“规则配置”中勾选/调整清洗、检测、修复等开关与阈值。
- 保存并开始处理，观察进度条与步骤指示。
- 在“审核界面”完成人工复核。

**章节来源**
- [internal/processor/pipeline.go:19-27](../internal/processor/pipeline.go#L19-L27)
- [internal/processor/pipeline.go:96-102](../internal/processor/pipeline.go#L96-L102)
- [internal/processor/pipeline.go:547-579](../internal/processor/pipeline.go#L547-L579)

### 提示词优化与自进化
- **动态提示词管理**：支持多版本提示词、热重载、自进化优化。
- **提示词来源**（按优先级）：
  1. **数据库存储**：Evolver 优化后的提示词自动保存到 `prompt_versions` 表。
  2. **文件系统**：从 `config/prompts/{prompt_name}*.txt` 文件加载（支持热重载）。
  3. **默认硬编码**：代码中的默认提示词，作为最终回退。
- **自进化监控**：当 API 错误率 >20% 或缓存命中率 <30% 时自动触发优化。
- **优化流程**：调用 `scripts/evolver.py` 分析错误模式，生成优化后的提示词。

建议步骤
- **文件方式**：在 `config/prompts/` 目录创建 `novel_repair_v1.0.0.txt` 等提示词文件。
- **数据库方式**：启用 Evolver 后，系统自动管理提示词版本。
- **查看当前提示词**：通过日志或数据库查询当前使用的提示词版本。

**章节来源**
- [internal/processor/prompt_manager.go:17-76](../internal/processor/prompt_manager.go#L17-L76)
- [internal/processor/model_repairer.go:71-79](../internal/processor/model_repairer.go#L71-L79)
- [internal/processor/evolver_monitor.go:35-120](../internal/processor/evolver_monitor.go#L35-L120)
- [scripts/evolver.py:1-250](../scripts/evolver.py#L1-L250)

## 依赖分析
- 运行时依赖：Gin、CORS、SQLite 驱动、dotenv、文本处理与 NLP 工具等。
- 构建与运行：Go 1.25，Linux/macOS/Windows 均可运行（注意路径分隔符）。

```mermaid
graph LR
A["go.mod<br/>依赖声明"] --> B["Gin/Web框架"]
A --> C["SQLite驱动"]
A --> D["dotenv/.env加载"]
A --> E["文本/NLP工具"]
```

**图表来源**
- [go.mod:5-13](../go.mod#L5-L13)

**章节来源**
- [go.mod:1-54](../go.mod#L1-L54)

## 性能考虑
- 数据库优化：启用 WAL 模式、设置同步级别、缓存大小、连接池与生命周期，减少锁竞争与 I/O 延迟。
- 处理并发：LLM 修复阶段采用智能分块与 Worker Pool 并发，配合缓存幂等性与断点续传，提升吞吐与稳定性。
- 前端交互：静态资源与路由分离，减少后端压力。

**章节来源**
- [internal/database/db.go:29-59](../internal/database/db.go#L29-L59)
- [internal/processor/pipeline.go:273-376](../internal/processor/pipeline.go#L273-L376)

## 故障排查指南
常见问题与解决思路
- 无法访问服务
  - 检查端口占用与防火墙，确认 .env 中 PORT 设置。
  - 使用冒烟测试脚本验证服务可用性与 API 根路径。
- 上传失败
  - 确认文件为 .txt 且未超过 MAX_FILE_SIZE。
  - 检查 DATA_DIR 权限与磁盘空间。
- 处理无响应
  - 查看 /api/files/:md5/status 获取当前步骤与进度。
  - 若处于 failed，使用 /api/files/:md5/resume 恢复处理。
- 审核项为空
  - 检查是否启用了向量检测与 LLM 修复。
  - 确认规则配置中的相似度阈值与模型参数合理。
- 配置未生效
  - 确认 .env 文件位置与权限，重启应用使配置生效。
  - 使用 Validate 校验端口、阈值、温度等参数范围。
- **提示词相关问题**
  - **提示词未加载**：检查 `config/prompts/` 目录权限，或查看数据库 `prompt_versions` 表。
  - **Evolver 未触发**：确认 `ENABLE_EVOLVER=true`，检查错误率和命中率阈值配置。
  - **提示词热重载失效**：确认 `HotReload` 配置为 true，检查文件系统监控权限。

辅助工具
- 冒烟测试：./testing/smoking_testing/run_smoke_tests.sh http://localhost:端口，自动化验证上传、处理、审核、删除等流程。
- **提示词调试**：查看日志中的"提示词管理器"相关记录，或查询数据库 `SELECT * FROM prompt_versions;`

**章节来源**
- [web/backend/handlers/files.go:18-71](../web/backend/handlers/files.go#L18-L71)
- [web/backend/handlers/files.go:200-251](../web/backend/handlers/files.go#L200-L251)
- [testing/smoking_testing/run_smoke_tests.sh:55-81](../testing/smoking_testing/run_smoke_tests.sh#L55-L81)
- [internal/processor/prompt_manager.go:17-76](../internal/processor/prompt_manager.go#L17-L76)

## 结论
通过本指南，你可以在本地快速完成环境准备、编译与运行，并使用浏览器前端完成上传、处理、审核与下载的完整流程。如需进一步扩展，可参考配置说明文档调整模型参数与部署方式。

## 附录

### 快速开始步骤清单
- 准备 Go 1.25+ 环境。
- 在项目根目录执行 `go mod tidy` 与 `go mod verify`。
- 复制并编辑 `.env`（端口、数据目录、模型参数等）。
- 编译项目：`make build`。
- 运行服务：`make dev`（开发调试）或 `make run`（生产部署）。
- 打开浏览器访问 `http://localhost:端口`，上传 .txt 文件并触发处理。
- 在前端“规则配置”中调整清洗/检测/修复策略，完成后生成最终文件并下载。

**章节来源**
- [Makefile](../Makefile)
- [scripts/run.sh:5-13](../scripts/run.sh#L5-L13)
- [doc/06-配置说明.md:1-146](../doc/06-配置说明.md#L1-L146)

### 基本使用示例（操作步骤）
- 上传文件
  - 在前端点击“上传文件”，选择 .txt 文件，提交后获得 {md5}。
- 触发处理
  - 在“规则配置”中设置清洗/检测/修复等开关与阈值，点击“保存并开始处理”。
- 查看进度
  - 在“处理进度”中查看步骤与整体进度条。
- 审核修改建议
  - 在“审核界面”对检测/修复建议进行“通过/拒绝/编辑/恢复”，或批量操作。
- 生成并下载最终文件
  - 审核完成后点击“完成审核并生成文件”，随后下载最终版本。

**章节来源**
- [web/frontend/index.html:35-222](../web/frontend/index.html#L35-L222)
- [web/backend/server.go:28-54](../web/backend/server.go#L28-L54)

### 提示词管理示例
- **使用文件系统管理提示词**
  ```bash
  # 创建提示词目录
  mkdir -p config/prompts
  
  # 创建提示词文件
  cat > config/prompts/novel_repair_v1.0.0.txt << 'EOF'
  你是一个专业的中文小说校对编辑。请修正以下段落中的错别字和语法错误，保持原文风格不变。只输出修正后的文本，无需解释。
  
  重要要求：
  1. 只修正明显的错别字和语法错误，不要改变原文意思
  2. 保持口语化表达，不要过度正式化
  3. 如果原文没有错误，直接返回原文
  4. 输出格式：只输出修正后的文本，不要添加任何额外说明
  
  示例：
  输入：她高兴及了，跑过去抱住他。
  输出：她高兴极了，跑过去抱住他。
  
  当前任务：请修正以下文本：
  EOF
  ```

- **启用自进化优化**
  ```env
  # 在 .env 文件中启用 Evolver
  ENABLE_EVOLVER=true
  EVOLVER_ERROR_RATE_THRESHOLD=0.2    # 错误率 >20% 时触发优化
  EVOLVER_HIT_RATE_THRESHOLD=0.3      # 命中率 <30% 时触发优化
  ```

- **查看当前提示词**
  ```bash
  # 查看日志中的提示词信息
  grep "提示词管理器" logs/app.log
  
  # 或通过数据库查询
  sqlite3 data/cleaning.db "SELECT prompt_version, source, LENGTH(prompt_content) as length FROM prompt_versions WHERE prompt_name='novel_repair' ORDER BY created_at DESC LIMIT 1;"
  ```

- **手动触发提示词优化**
  ```bash
  # 调用 Evolver 脚本
  python scripts/evolver.py \
    --prompt "当前提示词内容" \
    --context '{"error_type": "high_error_rate", "error_count": 15, "total_requests": 50}'
  ```

**章节来源**
- [internal/processor/prompt_manager.go:17-76](../internal/processor/prompt_manager.go#L17-L76)
- [scripts/evolver.py:1-250](../scripts/evolver.py#L1-L250)

### Docker 部署对比
- 传统部署（本机/服务器）
  - 优点：简单直接，便于调试与本地开发。
  - 注意：需自行管理 .env、DATA_DIR 权限与系统服务。
- Docker 部署（建议）
  - 优点：环境隔离、依赖统一、便于 CI/CD 与多环境复制。
  - 建议：挂载 DATA_DIR 与 .env，暴露端口，使用健康检查与日志采集。
  - 对照：传统部署与 Docker 部署在配置加载与静态资源路径上保持一致。

[本节为通用建议，不涉及具体源码实现，故不附加图表来源与章节来源]